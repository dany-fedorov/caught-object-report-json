...
* Apply configurable size budgets to complete reports and report arrays, measured in UTF-8 bytes by default.
* Bound child discovery with `maxDepth` and `maxChildren`; report shared objects once and reference their IDs, including cycles.
* Omit predictable fields by default and provide `restoreExpectedValues()` for a complete report.
* Fix custom child sources, throwing proxy traps, reporting-error context, and metadata preservation when deriving maker options.

### Breaking changes

* Reports use `corj/v0.12`; complete reports use `corj/v0.12-full`. Children link through `child_ids`, and `children_omitted` uses `max_depth`, `max_children`, or `max_size` codes. Truncation and cycle markers are `[truncated]` and `[circular]`.
* Use `makeCorj` / `makeCorjArray` instead of `makeCaughtObjectReportJson` / `makeCaughtObjectReportJsonArray` or the `bakeCorj` aliases.
* Construct `new CorjMaker(options?)` with partial options instead of `CorjMaker.withDefaults`; use `with` instead of `cloneWith`. Options are validated and frozen; the `*Entries` methods are removed.
* Rename `CORJ_MAKER_DEFAULT_OPTIONS` to `CORJ_DEFAULT_OPTIONS`, `maxChildrenLevel` to `maxDepth`, and `parseStackToArray` to `stackFormat: 'lines' | 'string'`. Stacks are arrays of lines by default.
* Replace `metadataFields` / `childrenMetadataFields` with `metadata`, and `onCaughtMaking` / `printWarningsOnUnhandledErrors` with `onError`. Unknown option names throw.
* Remove `asJsonFormatsToApply` / `asStringFormatsToApply`; `toCorjAsJson` / `toCorjAsString` methods are always tried first. Public type names use the `Corj` prefix; deprecated aliases are documented in the API reference.

See [Upgrading from v8](README.md#upgrading-from-v8) for the complete migration table.
