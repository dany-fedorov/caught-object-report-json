...
export const scenarios = [
  {
    ...
    run(corj) {
      const report = corj.makeReport(new Error('ordinary failure'));
  ...
  },
  {
    ...
    run(corj) {
      ...
      const report = corj.makeReport(outer);
  ...
  },
  {
    ...
    run(corj) {
      for (const [value, expected] of [
        ...
        const report = corj.makeReport(value);
        assert(
          report.typeof === expected,
          `typeof ${expected} reported as ${report.typeof}`,
        );
      }
      const nullReport = corj.makeReport(null);
  ...
  },
  {
    ...
    run(corj) {
      ...
      const report = corj.makeReport(new Lazy('boom'), {
        inspection: 'no-invoke',
      });
      ...
      assert(
        corj.makeReport(new Error('ordinary'), {
          inspection: 'no-invoke',
        }).stack.length > 0,
  ...
  },
  {
    ...
    run(corj) {
      const report = corj.makeReport(new Error('token sk-live-ABC'), {
  ...
  },
  {
    ...
    run(corj) {
      const report = corj.makeReport(new Error('identified'));
      assert(
      ...
      const off = corj.makeReport(new Error('anonymous'), {
  ...
  },
  {
    ...
    run(corj) {
      const outer = new Error('outer');
      outer.cause = new Error('inner');
      const rows = corj.makeReportArray(outer);
